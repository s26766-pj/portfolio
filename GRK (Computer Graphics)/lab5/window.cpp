...
void Window::InitGlewOrDie() {
    ...
    if (glDebugMessageCallback) {
        ...
        GLuint unused_ids = 0;
        glDebugMessageControl(GL_DONT_CARE, GL_DONT_CARE, GL_DONT_CARE, 0, &unused_ids, GL_FALSE);
    } else
...
void Window::KeyEvent(int key, int /*scancode*/, int action, int /*mods*/) {
    if (action == GLFW_RELEASE) {
        switch (key) {
            ...
            case GLFW_KEY_SPACE:
                current_model_index_ = (current_model_index_ + 1) % 4;
                break;
...
void Window::Run(void) {
    while (!glfwWindowShouldClose(window_)) {
        ...
        switch (current_model_index_) {
            case 0:
                star_.Draw(program_);
                break;
            case 1:
                estar_.Draw(program_);
                break;
            case 2:
                ecircle_.Draw(program_);
                break;
            case 3:
